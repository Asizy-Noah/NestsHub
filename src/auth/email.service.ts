...
export class EmailService {
  ...
  /**
   * Helper to generate the base HTML layout to maintain consistent branding
   */
  ...
  async sendVerificationApplicationManagerEmail(emails: string[], firstName: string, hostelName: string) {
    const content = `
    ...
    const mailOptions = {
      ...
      to: emails.join(', '), // Sends to both manager and hostel email
      subject: `Verification Application Received - ${hostelName}`,
      html: this.getBaseTemplate(content), // From previous implementation
    };
    await this.transporter.sendMail(mailOptions);
  }

  async sendVerificationApplicationAdminEmail(adminEmail: string, hostelName: string, phone: string) {
    const content = `
